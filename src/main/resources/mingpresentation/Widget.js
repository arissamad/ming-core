...
function Widget(widgetName, attachNow, settings) {
	...
	if(!_ming.widgetLibrary.hasOwnProperty(widgetName)) {
		throw new Error("Can't find widget: " + widgetName);
	}
	
	this.widget = _ming.widgetLibrary[widgetName].clone();
	if(this.widget == null) {
...
_ming.processWidgets = function() {
	
	var widgets = _ming.widgetHolder.children();
	for(var i=0; i<widgets.length; i++) {
		...
		_ming.widgetLibrary[widgetName] = widget;
	}
}
